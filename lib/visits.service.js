...
const syncPendingVisitCheckouts = async () => {
  ...
  for (let index = 0; index < pending.length; index += 1) {
    ...
    try {
      if (Boolean(item?.autoCheckedOut)) {
        // Ignore previously queued automatic checkout entries.
        syncedCount += 1;
        continue;
      }
...
export const autoCheckoutActiveVisits = async () => {
  try {
    ...
    // Only auto-checkout when server is really reachable.
    if (!(await canReachFirestoreServer())) return 0;

    const q = query(visitsCollection, where("checkOutTime", "==", null));
    const snapshot = await withTimeout(() => getDocs(q));
    if (snapshot.empty) return 0;
    if (snapshot?.metadata?.fromCache === true) return 0;

    // Skip visits that were checked in while offline.
    const eligibleDocs = snapshot.docs.filter((docSnap) => {
      const data = docSnap.data() || {};
      return data.checkInSource !== "offline";
    });

    if (eligibleDocs.length === 0) return 0;

    await Promise.all(
      eligibleDocs.map((docSnap) =>
        withTimeout(() =>
    ...
    const checkedOutIds = new Set(eligibleDocs.map((docSnap) => docSnap.id));
    const now = new Date();
    const cachedVisits = await loadCachedVisits();
    const nextCached = cachedVisits.map((visit) =>
      checkedOutIds.has(visit.id)
        ? {
            ...visit,
            status: "checked-out",
            checkOutTime: now,
    ...
    await saveCachedVisits(nextCached);

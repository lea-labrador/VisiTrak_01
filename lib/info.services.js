...
const FAST_SERVER_CHECK_TIMEOUT_MS = 2000;
...
export const fetchOfficesWithMeta = async () => {
  ...
  if (cachedOffices.length > 0 && hasUnknownOnlineState) {
    try {
      return await fetchOfficesFromServer(FAST_SERVER_CHECK_TIMEOUT_MS);
    } catch (error) {
      console.error("Error fetching offices (unknown network state):", error);
      return {
        items: cachedOffices,
        source: "cache",
        reason: "unknown-online-cache",
        onlineAvailable: false,

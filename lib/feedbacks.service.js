...
const normalizeShowNameToAdmin = (value) => {
...
const resolveFeedbackIdentity = (feedback = {}) => {
  const name = String(feedback?.name || "");
  const showNameToAdmin = normalizeShowNameToAdmin(feedback?.showNameToAdmin);
  const rawDisplayName = String(feedback?.displayName ?? "").trim();

  return {
    showNameToAdmin,
    displayName: rawDisplayName || (showNameToAdmin ? name : "Anonymous"),
  };
};

const serializeFeedback = (feedback) => ({
  ...feedback,
  ...resolveFeedbackIdentity(feedback),
...
const sanitizeFeedback = (feedback) => {
  ...
  const feedbackIdentity = resolveFeedbackIdentity(feedback);
